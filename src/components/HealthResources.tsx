...
export default function HealthResources() {
    const resources = [
        { title: "Drug Information", desc: "Detailed breakdown of 10k+ meds", icon: Pill, color: "bg-blue-50 text-blue-600" },
        { title: "Symptom Checker", desc: "AI-driven wellness analysis", icon: Activity, color: "bg-purple-50 text-purple-600" },
        { title: "Diet & Nutrition", desc: "Patient-specific diet plans", icon: Apple, color: "bg-green-50 text-green-600" },
        { title: "Health Articles", desc: "Verified medical research", icon: FileText, color: "bg-amber-50 text-amber-600" },
    ];
    ...
    return (
        <aside className="w-80 bg-zinc-50 border-l border-zinc-100 p-8 space-y-10 h-full sticky top-0 overflow-y-auto hidden xl:block">
                ...
                <div className="space-y-4">
                    {resources.map((res, i) => (
                        <motion.div
                            key={i}
                            whileHover={{ x: 4 }}
                            className="p-5 bg-white rounded-3xl border border-zinc-100 shadow-sm hover:shadow-md transition-all cursor-pointer group"
                        >
            ...
            <div className="p-6 bg-gradient-to-br from-blue-600 to-blue-700 rounded-[2rem] text-white space-y-4 shadow-xl shadow-blue-100">
